...
int parsecmd(char* __buf, int __bufsize, struct cmdlist* __head)
{
  ...
  while((word = strtok(cmd, " \t\n")) != NULL) { 
	if ((strcmp(word, "||") == 0) || (strcmp(word, "&&") == 0)) {
		struct cmdlist* newcmd = (struct cmdlist*) malloc(sizeof(struct cmdlist));
		curr->next = newcmd;
...
int executecmds(struct cmdlist* __head)
{
  ...
  while(curr != NULL){
	...
	if ((prevstat && (curr->conjuction == CONJOR)) || (!prevstat && (curr->conjuction == CONJAND))) {
		curr = curr->next;
		continue;
	}
    ...
    if(f == -1) {
      ...
      return RESERROR;
		...
		if (wait(&status) == -1) {
			e = errno;
			printf("Wait error: %s", strerror(e));
		}
		
		prevstat = !WEXITSTATUS(status);
		/*if (!WEXITSTATUS(status))
			printf("Wykonano poprawnie. Stat: %d, WIFE: %d, WSTAT: %d\n", status, WIFEXITED(status), WEXITSTATUS(status));
		else
			printf("Wykonano niepoprawnie\n");
		*/
	}
    curr = curr->next;
  }
